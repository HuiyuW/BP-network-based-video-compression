...
gscales = [0.06, 0.07, 0.1, 0.2, 0.4, 0.8, 1.0, 1.5, 2.0, 3.0, 4.0, 4.5];
num_gscale = numel(gscales);
bpp_intermean2 = zeros(1,num_gscale);
psnr_intermean2 = zeros(1,num_gscale);

bpp_intrahmean2 = zeros(1,num_gscale);
psnr_intrahmean2 = zeros(1,num_gscale);

% indexDeblocking = [1,26,28,32,36,35,37,38];%1,1.5
% indexDeblocking = [1,26,28,32,36,35,37,38];%1,1.5
indexDeblocking = [1,1,1,1,1,26,28,32,36,35,37,38];
...
final = deblocking_filter(image_rec,indexCurrent);
...
bpp_intrag = bit / (numel(image{1})/3);
bpp_intrah(1) = bpp_intrag;
% psnr_intrag = calcPSNR(image{1}, finalRGB);
psnr_intrag = calcPSNR(image_YCbCr{1}, final);
...
% 
    bpp_intermean2(scaleindex) = sum(bpp_inter)/length(bpp_inter);   
   psnr_intermean2(scaleindex) = sum(psnr_inter)/length(psnr_inter);
%    
   fprintf('q_scaleg: %4.2f  bit-rate: %8.4f bits/pixel  PSNR: %8.4fdB\n', qScale, bpp_intermean2(scaleindex), psnr_intermean2(scaleindex));
...
hold on
% plot(bpp_intrahmean, psnr_intrahmean,'p-','MarkerFaceColor','r','MarkerEdgeColor','k')
% hold on
...
% hold on
% plot(bpp_intrahmean6, psnr_intrahmean6,'p-','MarkerFaceColor','y','MarkerEdgeColor','k')
xlabel('bit/pixel')
ylabel('PSNR[dB]')
xlim([0.2 4])
title('D-R curve')
